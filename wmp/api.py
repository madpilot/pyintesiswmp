...
class API:
    ...
    def id(self) -> IdResult:
        return self._protocol.send("ID")

    def info(self) -> List[InfoResult]:
        return self._protocol.send("INFO")

    def set(self, ac_num: int, function: str, value: str) -> bool:
        return self._protocol.send("SET,%i:%s,%s" % (ac_num, function, value))

    def get(self, ac_num: int, function: str) -> CnfResult:
    ...
    def send_config(self, item: str, value: str) -> bool:
        return self._protocol.send("CFG:%s,%s" % (item, value))

    def get_config(self, item: str) -> CfgResult:
        return self._protocol.send("CFG:%s" % (item))

    def set_limits(self, function: str, range: List[Union[str, int]]) -> bool:
        range_str = map(lambda s: str(s), range)
        return self._protocol.send("LIMITS:%s,[%s]" % (function, ",".join(range_str)))

    def get_limits(self, function: str) -> LimitsResult:
